...
public class ExtractNodesPromptsTests
{
    ...
    public void BuildExtractAttributes_RendersExpectedPrompt()
    {
        ...
        var previous = new[]
        {
            CreateEpisode("Sam Rivera kept the 415-555-0142 number.", EpisodeType.Message)
        };
        ...
        var context = ExtractNodesPrompts.BuildExtractAttributesContext(node, episode, previous);
        var messages = ExtractNodesPrompts.BuildExtractAttributes(context);
        ...
        var expected = """
            ...
            "Sam Rivera works at Northwind."
            </MESSAGES>

            <ENTITY>
            {"name":"Sam Rivera","entity_types":["Entity","Person"],"attributes":{"phones":"415-555-0142"}}
            </ENTITY>
            """;
        Assert.Equal(expected, messages[1].Content);
    }
    ...
    public void BuildContext_RendersEntityTypesPreviousEpisodesAndCustomInstructions()
    {
        ...
        var expectedPreviousEpisodesJson =
            """[{"content":"Alice joined Acme Corp.","timestamp":"2026-01-01T12:00:00.0000000Z"}]""";
        var expectedEntityTypesJson =
            """[{"entity_type_id":0,"entity_type_name":"Entity","entity_type_description":"A specific, identifiable entity that does not fit any of the other listed types. Must still be a concrete, meaningful thing - specific enough to be uniquely identifiable. GOOD: a named entity not covered by the other types. BAD: \"luck\", \"ideas\", \"tomorrow\", \"things\", \"them\", \"everybody\", \"a sense of wonder\", \"great times\". When in doubt, do not extract the entity."},{"entity_type_id":1,"entity_type_name":"Person","entity_type_description":"A human person."}]""";

        Assert.Equal(expectedPreviousEpisodesJson, context.PreviousEpisodesJson);
        Assert.Equal(expectedEntityTypesJson, context.EntityTypesJson);
        ...
        var expected = $$"""
            ...
            <ENTITY TYPES>
            {{expectedEntityTypesJson}}
            </ENTITY TYPES>

            <PREVIOUS MESSAGES>
            {{expectedPreviousEpisodesJson}}
            </PREVIOUS MESSAGES>
