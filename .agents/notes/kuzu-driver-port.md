...
- Whether LadybugDB accepts the Python Kuzu Cypher shapes used by Graphiti, including
  `CREATE NODE TABLE IF NOT EXISTS`, `CREATE REL TABLE IF NOT EXISTS`, `MERGE ... SET`,
  `DETACH DELETE`, `WHERE x IN $list`, variable-length relationship paths, and `UNION`.
- Whether parameter names and binding use `$name` and accept dictionary-style values.
- CLR mappings for `DateTime`/`TIMESTAMP`, `List<string>`/`STRING[]`, `List<float>`/`FLOAT[]`,
  nullable timestamps, and JSON attribute strings.
...
- Tests in `Drivers/Ladybug/LadybugFoundationTests.cs` pin the foundation schema, save/get/delete/
  retrieve/load-embedding statements, individual bulk-save expansion, and record-mapper behavior.
...
- Tests in `Drivers/Ladybug/LadybugSearchStatementTests.cs` and
  ...
  foundation while asserting `LadybugGraphDriver` is still not an `ISearchGraphDriver`.
...
2. Add/use the LadybugDB package and decide how its connections/options should be represented in
   `GraphitiOptions`.
3. Add a concrete LadybugDB package adapter for `ILadybugQueryExecutor`. Keep `GraphProvider.Kuzu`
   ...
   direct mapping.
4. Prove the full C# Saga schema/save/get projections and entity-edge `reference_time` projections
   against the real LadybugDB backend as part of the first runtime execution slice.
5. Add optional native-gated integration smoke tests.
6. Wire LadybugDB through `GraphitiServiceCollectionExtensions.CreateGraphDriver` and make it the
   default first provider option when safe.
7. Revisit interim Kuzu query/filter helpers and move provider-specific behavior into the driver when
   appropriate.
8. Add focused driver tests and parity tests against Python Kuzu behavior.
9. Once the driver is stable, finish the Kuzu-to-LadybugDB terminology transition.
10. Update this note, `decisions.md`, `handoff.md`, and `roadmap.md` when the driver lands.
