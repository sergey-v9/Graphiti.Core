...
- LadybugDB/Kuzu foundation helpers may land before final provider naming/wiring decisions, but they
  ...
  and `GraphProvider.Kuzu` remains unsupported by core provider validation. A first factory-backed
  `Graphiti` ingest/search/removal workflow is proved, but broader workflow coverage and the
  driver-facing LadybugDB naming decision are still required before core provider wiring.
