...
|---|---|---|
| `e6c3af9` FalkorDB all-stopword full-text fix | Already aligned | Ladybug returns no candidates for the same stopword-only probe; no RediSearch syntax is shared |
| `7db4031` FalkorDB backtick/empty-token full-text fix | ADOPTED semantically | Ladybug keeps searchable queries verbatim (including `` `add_episode` ``) and now skips punctuation-only input that the engine otherwise treats as a broad match |
...
| `784782c` group-scoped community cleanup | ADOPTED | Explicit group rebuilds delete only those groups' communities using typed deletion; `null`/empty still clear all on built-in drivers |
| `a96309b` return entity-edge `reference_time` | Already aligned | Ladybug projections/map and InMemory clones hydrate the stored timestamp; runtime assertions pin it |
| `2453209` consume BFS relationship hits directly | Already aligned | Ladybug projects the intermediate edge node directly and InMemory returns the stored traversal edge |
| `b0ec6d9` preserve relationship direction in incident/BFS reads | Already aligned | Target-side Ladybug reads and InMemory BFS preserve the original source/target endpoints |
| `d9a2db9` provider/search consolidation | ADOPTED / N/A | Scoped typed community cleanup is adopted; shared edge projection/BFS behavior was already aligned; FalkorDB/RediSearch mechanics are N/A |
| `d40da88` edge cross-encoder shortlist | ADOPTED | RRF fuses text/vector/BFS into a bounded `2 * limit` shortlist before cross-encoding |
...
| Communities | `build_communities` | `BuildCommunitiesAsync` | OK | Community summary reduction preserves raw entity summaries, including blank strings, like Python. On built-in drivers, omitted group IDs discover all entity groups, including the default empty-string group; explicit `[]` clears existing communities and builds none like Python. Rebuilding a non-empty group selection removes and recreates only those groups' community nodes, preserving unrelated groups and non-community nodes with colliding UUIDs. Explicit group order and intra-group read order are preserved for returned communities and membership edges |
