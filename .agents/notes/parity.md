...
2026-06-19 follow-up: a public/extensibility audit found no result-composition code slice. C# keeps
fresh search recipe instances instead of reproducing upstream's mutable singleton recipes and skips
custom driver BFS calls for null/empty origins or nonpositive depth instead of surfacing no-op calls.
Both are documented C# API hardening decisions; recipe values, wire values, and built-in search results
remain aligned for equivalent inputs.
