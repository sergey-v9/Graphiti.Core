...
## ⚠ User-gated — confirm with Sergey before acting (supervisor review 2026-06-17)

Some recent work was generated by following this roadmap into decisions that are **Sergey's to make,
not the agent's**. Treat the following as PENDING SERGEY'S CONFIRMATION — do not extend them or treat
them as settled direction, and surface them rather than self-authorizing from this file:

1. **CI was out of scope.** Two GitHub Actions lanes (`.github/workflows/core-only.yml`, `full.yml`)
   were added even though "CI for the Graphiti package is out of scope" is a standing directive
   (still stated in `upstream-sync-procedure.md`). Do not add or expand CI further until Sergey
   confirms CI is now in scope. (The Linux full lane does not actually work yet — see the
   `full.yml` note about the Ladybug Linux FTS extension ABI mismatch under `~/.lbdb/extension`.)
2. **Publishing the LadybugDB binding was decision-gated.** The fork binding was published to the
   public `sergey-v9/ladybug-dotnet` GitHub Packages feed and `NuGet.config` was repointed there,
   dropping the local offline artifacts feed; an earlier standing rule was "do not push the ladybug
   repo remotely." Local/CI restore now needs a `github_ladybug` credential (`read:packages`).
   Confirm the publish approach with Sergey, and **keep the local offline feed as a restore fallback**
   so offline dev does not require a credential.
3. **Neo4j removal is not decided.** Neo4j is SUPPORTED legacy compatibility, kept from regressing.
   Its removal is a user-gated decision Sergey has not made — do not frame it as "expected to be
   removed" or schedule a removal milestone without an explicit go-ahead.
4. **Release publishing / versioning** (the Graphiti packages themselves, the 2.0.0 line, alpha→beta
   cadence, metapackage shape) is Sergey's call.

The genuinely in-scope standing work needs no sign-off: tracking upstream Python parity, LadybugDB
provider hardening, search/pipeline correctness, tests, and docs.
