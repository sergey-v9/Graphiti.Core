...
3. Port the LadybugDB provider.
   ...
   runtime-backed proof with attribution lookup and search. Next provider work should broaden
   workflow coverage, add runtime-driven host options only when needed, and avoid marking core DI
   support complete too early. Details are in `kuzu-driver-port.md`.
...
- A test-only LadybugDB package/runtime proof now uses private `LadybugDB` / `LadybugDB.Native`
  ...
  coalescing is proved; core still has no LadybugDB package reference and `GraphProvider.Kuzu`
  remains unsupported.
...
- Good next provider slice after the initial runtime-backed ingest/search/removal/triplet/bulk proof:
