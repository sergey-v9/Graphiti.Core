...
**DONE (2026-06-27).** The package remains `2.0.0-alpha.1`. The non-gated release-surface work is
complete: public-surface decisions are settled, the API snapshot is regenerated, package/RID claims are
audited, and a release-readiness pack + fresh-consumer dry run is green. This plan stopped **before**
the user-gated line: no version stamp and no publish were performed.

## Why this is the next stream

The HIGH productionization axes are done and green (1032/4/1036; linux-x64 smoke proven; live canary
...
- [x] **B. Attribute `MaxLength` + required-field carve-out — close or formally document.** Python's
  ...
  decision, not a gap.
- [x] **C. Public-API freeze pass.** With A/B settled, do a deliberate read of the full
...
- [x] **D. Package metadata + RID truth.** Audit the `.nuspec`/csproj metadata (description, tags,
...
- [x] **E. Clean pack + fresh-consumer dry run.** Run `.\eng\Verify-GraphitiCore.ps1` end to end
...
`.\eng\Verify-GraphitiCore.ps1` is green on win-x64 as the release-readiness dry run (2026-06-27):
`1032` passed, `4` skipped, `1036` total; format verification, warning-clean build, pack, and fresh
package-consumer smoke included. The
