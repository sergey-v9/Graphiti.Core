...
{
    internal readonly record struct CombinedExtractionContext(
    ...
    internal static CombinedExtractionContext BuildContext(
    ...
    internal static CombinedExtractionContext BuildContext(
        ...
        string? customExtractionInstructions)
    {
        return new CombinedExtractionContext(
            BuildEpisodeContent(episodes),
    ...
    internal static Message[] BuildExtractMessage(in CombinedExtractionContext context)
    {
    ...
    private static string BuildEpisodeContent(IReadOnlyList<EpisodicNode> episodes)
    {
        var values = new List<(string Content, DateTime? ValidAt)>(episodes.Count);
        for (var i = 0; i < episodes.Count; i++)
        {
            values.Add((episodes[i].Content, episodes[i].ValidAt));
        }

        return TextUtilities.ConcatenateEpisodes(values);
    }
}
