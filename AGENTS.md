...
release-versioning/publishing is parked (user-gated). **Plans 05–11 are all complete — the backlog is
exhausted and there is no open plan.** The forward posture is **maintenance**: run the upstream-parity
sweep (`upstream-sync-procedure.md`) to stay current with `getzep/graphiti`, and apply opportunistic,
parity-safe modernization as the language/runtime move — every change behavior-, wire-, and
schema/cache-preserving, hot-path changes benchmark-first. A genuinely new direction comes from Sergey.
